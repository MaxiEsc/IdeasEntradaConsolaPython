#Ejemplo de tipos de datos
from conversion_tipos_datos import numero_entero

#Ejemplo de concatenacion y sumas de valores
numero1_cadena = '10'
print(f'Numero 1 en cadena {numero1_cadena}')
numero2_cadena = '20'
print(f'Numero 1 en cadena {numero2_cadena}')
resultado = numero2_cadena + numero2_cadena

print(f'Concatenacion: {resultado}')

#Convertir a tipos enteros
numero1_entero = int(numero1_cadena)
numero2_entero = int(numero2_cadena)

resultado = numero1_entero + numero2_entero
print(f'Suma: {resultado}')

